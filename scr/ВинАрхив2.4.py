...
def pack_tar():
    def _pack_tar():
        ...
        if not files:
            return
        tar_file = filedialog.asksaveasfilename(defaultextension='.rar', filetypes=[('TAR ФАЙЛ', '*.tar')])
        if not zip_file:
            return
        with tarfile.open(tar_file, 'w') as tar:
            tar.add(file)
    threading.Thread(target=_pack_tar).start()

def about_tar():
    def _about_tar():
        tar_path = filedialog.askopenfilename(filetypes=[('TAR ФАЙЛ', '*.tar')])
        with tarfile.open(tar_path, 'r') as tar:
            file_names = tar.getnames()
            for file_name in file_names:
                showinfo(title="Информация", message=f"{file_name}")
    threading.Thread(target=_about_tar).start()
root = tk.Tk()
root.title("ВинАрхив")
...
btn_unpack_tar.pack()
btn_pack_tar = tk.Button(root, text="Упаковка TAR", command=pack_tar)
btn_pack_tar.pack()
btn_about_tar = tk.Button(root, text="Содержимое TAR", command=about_tar)
btn_about_tar.pack()
